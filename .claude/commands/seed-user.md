...
## Step 1 — Generate a realistic random Indian user
...
## Step 2 — Ensure email is unique

Check if the generated email already exists in the `users` table. If it does, regenerate until unique.

## Step 3 — Insert the user

Insert the user into the database using the same `get_db()` pattern found in `db.py`.
